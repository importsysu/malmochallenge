...
EPSILON = 0.1# the probability of agent tacking random action
...
class BayesAgent(BaseAgent):
    ...
    # Goal Swapping
    #==============================================================================
    def collecting(self, obs, action, reward, next_obs, done, step):
        state = self.state_shaping(obs)
        next_state = self.state_shaping(next_obs)
        r = self.reward_shaping(state, reward)
    ...
    def training(self):
        batch = self.sampling()
        return self.update_model(batch)
    ...
    # it's noisy but still can be rough prior
    #==============================================================================
    ...
    def update_collaborator(self, next_obs, reward):
        ...
        elif ori_pig_dist > new_pig_dist and ori_leave1_dist >= ori_leave2_dist:
    ...
    def save_replay_buffer(self):
        with open('replay_buffer.msg','w') as f:
            f.truncate()
            replay_buffer = {'replay_buffer': [x for x in self.replay_buffer]}
...
class Critic(object):
    def __init__(self, state_dims, goal_dims, sess):
        ...
        self.goal_dims = goal_dims
        self.build_model()
        self.new_pars = tf.get_collection(tf.GraphKeys.VARIABLES, scope='new_network')
        self.tar_pars = tf.get_collection(tf.GraphKeys.VARIABLES, scope='target_network')
    ...
    def build_model(self):
        ...
        with tf.variable_scope('new_network'):
            ...
            with tf.variable_scope('layer_4'):
                ...
                tf.histogram_summary("W_fc4", self.W_fc3)
                tf.histogram_summary("b_fc4", self.b_fc3)
            with tf.variable_scope('layer_5'):
                ...
                self.prediction = tf.reduce_sum(self.qvalue * self.goal_input, reduction_indices=1)
                tf.histogram_summary("W_fc5", self.W_fc5)
                tf.histogram_summary("b_fc5", self.b_fc5)
        ...
        with tf.variable_scope('target_network'):
            with tf.variable_scope('layer_1'):
                ...
                tf.histogram_summary("_W_fc1", self._W_fc1)
                tf.histogram_summary("_b_fc1", self._b_fc1)
            ...
            with tf.variable_scope('layer_2'):
                ...
                tf.histogram_summary("_W_fc2", self._W_fc2)
                tf.histogram_summary("_b_fc2", self._b_fc2)
            ...
            with tf.variable_scope('layer_3'):
                ...
                tf.histogram_summary("_W_fc3", self._W_fc3)
                tf.histogram_summary("_b_fc3", self._b_fc3)
            with tf.variable_scope('layer_4'):
                ...
                tf.histogram_summary("_W_fc4", self._W_fc4)
                tf.histogram_summary("_b_fc4", self._b_fc4)
            with tf.variable_scope('layer_5'):
                ...
                self._prediction = tf.reduce_sum(self._qvalue * self.goal_input, reduction_indices=1)
                tf.histogram_summary("_W_fc5", self._W_fc5)
                tf.histogram_summary("_b_fc5", self._b_fc5)
    ...
    def update(self, state, goal, reward, next_state, step):#using double q learning
        ...
        summary, train, l, qvalue = self.sess.run([self.merged, self.train_step, self.loss, self.qvalue], \
